...
### 3.1 Semua client yang ada **HARUS** menggunakan konfigurasi IP dari DHCP Server.

Nomor 1, edit network config **Alabasta, Loguetown, Skypie, Tottoland** menjadi
...
Luffy dan Zoro berencana menjadikan Skypie sebagai server untuk jual beli kapal yang dimilikinya

dengan alamat IP yang tetap dengan IP [prefix IP].3.69 
...
Pada Loguetown, proxy harus bisa diakses dengan nama jualbelikapal.yyy.com dengan port
yang digunakan adalah 5000
...
export http_proxy=http://10.38.2.3:5000 lynx http://its.ac.id

htpasswd -cm /etc/squid/passwd luffybelikapalIUP1 luffy_IUP1 luffy_IUP1

htpasswd -m /etc/squid/passwd zorobelikapalIUP1 zoro_IUP1 zoro_IUP1

env | grep -i proxy unset http_proxy
```
...
```
# /etc/bind/kaizoku/super.franky.IUP1.com
;
